...
#
# df = pd.read_csv('input/db_files/sitedb.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
#
# header = list(df.columns)
# header_low=[]
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns=header_low
# df.to_sql('sitedb_2g', engine)
# # for col in header_low:
# #     executeQuery(f'select "{col}" from sitedb_2g')
#
#
# df = pd.read_csv('input/db_files/sitedb_temp1.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low=[]
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns=header_low
# df.to_sql('sitedb_2g_temp1', engine)
#
#
# df = pd.read_csv('input/db_files/CNRH_20200817.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low = []
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns = header_low
# df.to_sql('cnrh_2g', engine)
#
#
# df = pd.read_csv('input/db_files/sitedb_temp.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low = []
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns = header_low
# df.to_sql('sitedb_2g_temp', engine)
#
# df = pd.read_csv('input/db_files/network_all_bbh_kpi_20200817.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low = []
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns = header_low
# df.to_sql('bbh_2g', engine)
#
# df = pd.read_csv('input/db_files/network_all_daily_kpi_20200817.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low = []
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns = header_low
# df.to_sql('day_2g', engine)
#
# df = pd.read_csv('input/db_files/network_all_hourly_kpi_20200817.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low = []
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns = header_low
# df.to_sql('hour_2g', engine)
#
# df = pd.read_csv('input/db_files/MCL_20200817.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low = []
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns = header_low
# df.to_sql('mcl', engine)
#
# df = pd.read_csv('input/db_files/network_all_nbh_kpi_20200817.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low = []
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns = header_low
# df.to_sql('nbh_2g', engine)
#
# df = pd.read_csv('input/db_files/core_hlr_nsub_20200817.csv')
# engine = create_engine('postgresql://'+config['DBUser']+':'+config['DBPwd']+'@'+config['DBHost']+':'+config['DBPort']+'/'+config['Database'])
# header = list(df.columns)
# header_low = []
# for col in header:
#     header_low.append(str(col).lower().replace('-', '_'))
# df.columns = header_low
# df.to_sql('core_hlr_nsub', engine)
# print('done')
...
def cnrh():
    # cnrh
    attached = executeQuery("select sum(attached) from cnrh_2g where (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6))= (SELECT left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6) FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1)")
    detached = executeQuery("select sum(Total_VLR) - sum(attached) from cnrh_2g where (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6))= (SELECT left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6) FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1)")
    total_vlr = executeQuery("select sum(Total_VLR) from cnrh_2g where (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6))= (SELECT left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6) FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1)")
    ...
    sms_delivered_during_busy_hour = executeQuery("select sum(SMS_Delivered_during_Busy_Hour) from cnrh_2g where (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6))= (SELECT left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6) FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1)")
    network_busy_hour_start_time = executeQuery("SELECT (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6)) hr FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1")
    bh_traffic_erl = executeQuery("select trf from (select sum(BH_Traffic_Erl) trf from cnrh_2g group by Period_start_time) f order by trf desc limit 1")
    ...
    peak_processor_load_24hrs = executeQuery("select avg(max_peak) from (select ENSS_name, max(Peak_Processor_Load) AS max_peak from cnrh_2g group by ENSS_name) dd")
...
def network():
    ...
    bhca_sub = executeQuery("select((SELECT sum(Switch_BHCA) FROM mcl where etst_id in('IEX','ORG','Reliable','All Types of Calls') and (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6))=(SELECT (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6)) hr FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1) )/(SELECT sum(Attached) FROM cnrh_2g where (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6))=(SELECT (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6)) hr FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1)))")

    #nbh
    total_nbh_traffic_erlangs = executeQuery("SELECT (sum(Traffic_Full_Rate_Erlangs)+sum(Traffic_Half_Rate_Erlangs)) FROM nbh_2g")
    nbh_traffic_full_rate_erlangs = executeQuery("SELECT sum(Traffic_Full_Rate_Erlangs) FROM nbh_2g")
    nbh_traffic_half_rate_erlangs = executeQuery("SELECT sum(Traffic_Half_Rate_Erlangs) FROM nbh_2g")
    nbh_traffic_amr_full_rate_erlangs = executeQuery("SELECT sum(Traffic_AMR_Full_Rate_Erlangs) FROM nbh_2g")
    nbh_traffic_amr_half_rate_erlangs = executeQuery("SELECT sum(Traffic_AMR_Half_Rate_Erlangs) FROM nbh_2g")
    nbh_period = executeQuery("SELECT left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6)  FROM nbh_2g  limit 1")
    random_access_success_rate = eval(str(executeQuery("select ((SELECT sum(Random_Acc_Succ_Rate_Nom) FROM nbh_2g)/(SELECT sum(Random_Acc_Succ_Rate_Denom) FROM nbh_2g))*100")))
    sdcch_blocking_nbh = eval(str(executeQuery("select((SELECT sum(SDCCH_Blocking_Nom) FROM nbh_2g)/(SELECT sum(SDCCH_Blocking_Denom) FROM nbh_2g ))*100")))
    sdcch_drop_nbh = eval(str(executeQuery("select((SELECT sum(SDCCH_Drop_Nom) FROM nbh_2g)/(SELECT sum(SDCCH_Drop_Denom) FROM nbh_2g ))*100")))
    tch_blocking_user_percieved = eval(str(executeQuery("select((SELECT sum(TCH_Blocking_Nom) FROM nbh_2g)/(SELECT sum(TCH_Blocking_Denom) FROM nbh_2g ))*100")))
    tch_assignment_success_rate_nbh = eval(str(executeQuery("select  case when ((SELECT sum(TCH_Assig_Success_Rate_Nom) FROM nbh_2g)/(SELECT sum(TCH_Assig_Success_Rate_Denom) FROM nbh_2g))*100 >  100 then 100 else ((SELECT sum(TCH_Assig_Success_Rate_Nom) FROM nbh_2g)/(SELECT sum(TCH_Assig_Success_Rate_Denom) FROM nbh_2g))*100 end;")))
    tch_drop_nbh = eval(str(executeQuery("select((SELECT sum(TCH_Drop_Nom) FROM nbh_2g)/(SELECT sum(TCH_Drop_Nom_Denom) FROM nbh_2g ))*100")))
    handover_success_rate = eval(str(executeQuery("select((SELECT sum(Handover_Success_Rate_Nom) FROM nbh_2g)/(SELECT sum(Handover_Success_Rate_Denom) FROM nbh_2g ))*100")))
    location_update_success_rate_nbh = eval(str(executeQuery("select((SELECT sum(Location_Update_Success_Rate_Nom) FROM cnrh_2g where (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6))=(SELECT (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6)) hr FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1))/(SELECT sum(Location_Update_Success_Rate_Denom) FROM cnrh_2g where (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6))=(SELECT (left(split_part(Period_start_time, ' ', 2),length(split_part(Period_start_time, ' ', 2))-6)) hr FROM cnrh_2g group by period_start_time order by sum(BH_Traffic_Erl) desc limit 1)))*100")))
    trunk_route_utilization_rate_nbh = eval(str(executeQuery("select 100*SUM(Route_Utilization_nom)/SUM(Route_Utilization_denom) trf from cnrh_2g group by Period_start_time order by sum(BH_Traffic_Erl) desc limit 1")))
    trunk_route_congestion_rate_nbh = eval(str(executeQuery("select 100*SUM(Route_Congestion_nom)/SUM(Route_Congestion_denom) trf from cnrh_2g group by Period_start_time order by sum(BH_Traffic_Erl) desc limit 1")))
    tch_setup_success_rate_nbh = eval(str(executeQuery("select case when ((SELECT sum(TCH_Assig_Success_Rate_Nom) FROM nbh_2g)/(SELECT sum(TCH_Assig_Success_Rate_Denom) FROM nbh_2g))*100 >  100 then 100  else ((SELECT sum(TCH_Assig_Success_Rate_Nom) FROM nbh_2g)/(SELECT sum(TCH_Assig_Success_Rate_Denom) FROM nbh_2g))*100 end")))
    erlang_mins_per_drop = eval(str(executeQuery("select((SELECT sum(Total_Traffic_Erlangs) FROM nbh_2g)/(SELECT sum(TCH_Drop_Nom)FROM nbh_2g ))*60")))
    tbf_dropped_rate = eval(str(executeQuery("select((SELECT sum(Percentage_TBF_drop_Nom) FROM nbh_2g)/(SELECT sum(Percentage_TBF_drop_Denom) FROM nbh_2g))*100")))
    cells_having_tbf_dropped_rate_lt_7 = eval(str(executeQuery("select((SELECT count(Percentage_TBF_dropped) FROM nbh_2g where Percentage_TBF_dropped<7)/(SELECT count(Percentage_TBF_dropped)FROM nbh_2g))*100")))
    avg_ul_throughout_per_tbf_gprs = eval(str(executeQuery("SELECT avg(Average_UL_throughput_per_TBF_GPRS) FROM nbh_2g")))
    avg_dl_throughout_per_tbf_gprs = eval(str(executeQuery("SELECT avg(Average_DL_throughput_per_TBF_GPRS) FROM nbh_2g")))
    avg_ul_throughout_per_tbf_edge = eval(str(executeQuery("SELECT avg(Average_UL_throughput_per_TBF_EDGE) FROM nbh_2g")))
    avg_dl_throughout_per_tbf_edge = eval(str(executeQuery("SELECT avg(Average_DL_throughput_per_TBF_EDGE) FROM nbh_2g")))
    radio_nw_util = eval(str(executeQuery("select((SELECT sum(Total_Traffic_Erlangs) FROM nbh_2g)/((SELECT sum(Equipped_Erlang_Capacity_forVoice) FROM sitedb_2g  where MACRO_IBS='MACRO')+(SELECT case when sum(Equipped_Erlang_Capacity_forVoice) is null then 0  else sum(Equipped_Erlang_Capacity_forVoice) end   FROM sitedb_2g  where MACRO_IBS='MICRO')+(SELECT case when sum(Equipped_Erlang_Capacity_forVoice) is null then 0 else sum(Equipped_Erlang_Capacity_forVoice) end FROM sitedb_2g  where MACRO_IBS='IBS')))*100")))
    ...
    networkDict['network_avail_total_radio_24hrs'] = network_avail_total_radio_24hrs
    networkDict['total_cell_outage_mins'] = total_cell_outage_mins
    networkDict['unplanned_cell_outage_mins'] = unplanned_cell_outage_mins
    networkDict['radio_network_avail_excluding_planned'] = radio_network_avail_excluding_planned
    # networkDict['total_cell_outage_mins_07_23'] = total_cell_outage_mins_07_23
    # networkDict['unplanned_cell_outage_mins_07_23'] = unplanned_cell_outage_mins_07_23
    networkDict['radio_network_availability_excluding_planned'] = radio_network_availability_excluding_planned
    networkDict['me_sub_switch_nbh_vlr_attached'] = me_sub_switch_nbh_vlr_attached
    networkDict['bhca_sub'] = bhca_sub

    networkDict['total_nbh_traffic_erlangs'] = total_nbh_traffic_erlangs
    networkDict['nbh_traffic_full_rate_erlangs'] = nbh_traffic_full_rate_erlangs
    networkDict['nbh_traffic_half_rate_erlangs'] = nbh_traffic_half_rate_erlangs
    networkDict['nbh_traffic_amr_full_rate_erlangs'] = nbh_traffic_amr_full_rate_erlangs
    networkDict['nbh_traffic_amr_half_rate_erlangs'] = nbh_traffic_amr_half_rate_erlangs
    networkDict['nbh_period'] = nbh_period
    networkDict['random_access_success_rate'] = random_access_success_rate
    networkDict['sdcch_blocking_nbh'] = sdcch_blocking_nbh
    networkDict['sdcch_drop_nbh'] = sdcch_drop_nbh
    networkDict['tch_blocking_user_percieved'] = tch_blocking_user_percieved
    networkDict['tch_assignment_success_rate_nbh'] = tch_assignment_success_rate_nbh
    networkDict['tch_drop_nbh'] = tch_drop_nbh
    networkDict['handover_success_rate'] = handover_success_rate
    networkDict['location_update_success_rate_nbh'] = location_update_success_rate_nbh
    networkDict['trunk_route_utilization_rate_nbh'] = trunk_route_utilization_rate_nbh
    networkDict['trunk_route_congestion_rate_nbh'] = trunk_route_congestion_rate_nbh
    networkDict['tch_setup_success_rate_nbh'] = tch_setup_success_rate_nbh
    networkDict['erlang_mins_per_drop'] = erlang_mins_per_drop
    networkDict['tbf_dropped_rate'] = tbf_dropped_rate
    networkDict['cells_having_tbf_dropped_rate_lt_7'] = cells_having_tbf_dropped_rate_lt_7
    networkDict['avg_ul_throughout_per_tbf_gprs'] = avg_ul_throughout_per_tbf_gprs
    networkDict['avg_dl_throughout_per_tbf_gprs'] = avg_dl_throughout_per_tbf_gprs
    networkDict['avg_ul_throughout_per_tbf_edge'] = avg_ul_throughout_per_tbf_edge
    networkDict['avg_dl_throughout_per_tbf_edge'] = avg_dl_throughout_per_tbf_edge
    networkDict['radio_nw_util'] = radio_nw_util
